...
from core.utils.utilidades_fechas import formatear_fecha, mes_nombre, formatear_fecha_simple
...
class ReportePacienteService:
    ...
    def __dibujarEpicrisisFormatoHojaHospitalizacion2026(pdf, ancho, alto, paciente, ingreso):
        pdf.showPage()
        ReportePacienteService.__dibujar_encabezado_epicrisis(pdf, ancho, 755)
        pdf.setFont("Helvetica-Bold", 14)
    ...
    def GenerarFormatoHojaHospitalizacion2026(data, usuario):
        try:
            ...
            # 3. DIBUJO DOCUMENTO
            #pagina 1 
            ReportePacienteService.__dibujarEstructuraHojaHospitalizacion2026(pdf,ancho,alto)
            ReportePacienteService.__dibujarEtiquetasEstaticasHojaHospitalizacion2026(pdf,ancho,alto)
            altura_bloque3 = ReportePacienteService.__dibujarDatosPacienteHojaHospitalizacion2026(pdf,ancho,alto,paciente)
            
            ReportePacienteService.__dibujarDatosIngresoHojaHospitalizacion2026(pdf,ancho,alto,altura_bloque3,acompaniante,padres,ingreso,usuario_creacion)
            ReportePacienteService.__dibujarPiePaginaFormatoHojaHospitalizacion2026(pdf, ancho, alto, usuario)
            #PAGINA2
            ReportePacienteService.__dibujarAutorizacionFormatoHojaHospitalizacion2026(pdf, ancho, alto, ingreso, paciente, padres, acompaniante)
            ReportePacienteService.__dibujarPiePaginaFormatoHojaHospitalizacion2026(pdf, ancho, alto, usuario)
            #pagina 3
            ReportePacienteService.__dibujarEpicrisisFormatoHojaHospitalizacion2026(pdf, ancho, alto, paciente, ingreso)
            ReportePacienteService.__dibujarPiePaginaFormatoHojaHospitalizacion2026(pdf, ancho, alto, usuario)
    ...
    @staticmethod
    def _dibujarEtiquetasEstaticasEntregaCadaver(pdf, ancho, alto):
        pdf.setFont("Helvetica-Bold", 18)
        pdf.drawCentredString(ancho / 2, alto - 120, "AUTORIZACIÓN")

        pdf.setStrokeColorRGB(0.4, 0.4, 0.4)
        pdf.line(236, alto - 125, 373, alto - 125)

        pdf.drawCentredString(ancho / 2, alto - 145, "ENTREGA DE CADAVERES")
    ...
    @staticmethod
    def _dibujarDatosEntregaCadaver(pdf, defuncion, expediente, usuario, alto):

        # Nombre paciente
        nombreCompleto = formatear_nombre_completo(
            defuncion.paciente.primer_nombre,
            defuncion.paciente.segundo_nombre,
            defuncion.paciente.primer_apellido,
            defuncion.paciente.segundo_apellido
        )[:38]

        pdf.drawString(240, alto - 260, nombreCompleto)

        # Expediente
        pdf.drawString(240, alto - 290, expediente)

        # Fecha nacimiento
        pdf.drawString(
            240,
            alto - 320,
            formatear_fecha_simple(defuncion.paciente.fecha_nacimiento)
        )
        ...
        pdf.drawString(
            240,
            alto - 350,
            formatear_fecha_simple(defuncion.fecha_defuncion)
        )
        ...
        pdf.drawString(
            110,
            alto - 662,
            f"{usuario.first_name.upper()} {usuario.last_name.upper()}"[:20]
        )

        # Responsable
        if defuncion.reponsable_nombre and defuncion.reponsable_dni:
            pdf.drawString(240, alto - 450, defuncion.reponsable_nombre)
            pdf.drawString(395, alto - 662, defuncion.reponsable_dni)
            pdf.drawString(340, alto - 677, defuncion.reponsable_nombre)


    @staticmethod
    def generar_entrega_cadaver_pdf(defuncion, expediente, usuario):
        ...
        # Etiquetas
        ReportePacienteService._dibujarEtiquetasEstaticasEntregaCadaver(pdf, ancho, y)

        # Datos dinámicos
        ReportePacienteService._dibujarDatosEntregaCadaver(
            pdf, defuncion, expediente, usuario, alto
        )
        ...
        ReportePdfBaseService.dibujar_pie_pagina_carta(
            ...
            f"{usuario.first_name.upper()} {usuario.last_name.upper()}",
            1,
            1
        )
    ...
    @staticmethod
    def _dibujarEtiquetasEstaticasObito(pdf, ancho, alto):
        pdf.setFont("Helvetica-Bold", 18)
        pdf.drawCentredString(ancho / 2, alto - 120, "AUTORIZACIÓN")
        ...
        pdf.drawCentredString(ancho / 2, alto - 145, "ENTREGA DE OBITO")
        pdf.line(190, alto - 150, 425, alto - 150)

        pdf.setFont("Helvetica", 12)
        pdf.drawString(70, alto - 230, "NOMBRE DE LA MADRE:")
        pdf.drawString(70, alto - 260, "EXPEDIENTE DE LA MADRE:")
        pdf.drawString(70, alto - 290, "FECHA DE OBITO:")
        
        pdf.drawString(70, alto - 390, "PERSONA RESPONSABLE:")
    ...
    @staticmethod
    def _dibujarDatosObito(pdf, defuncion, expediente, usuario, alto):
        ...
        pdf.drawString(X_DATO, alto - 260, nombreCompleto)

        # Expediente
        pdf.drawString(X_DATO, alto - 290, expediente)
        ...
        # Responsable
        if defuncion.responsable_nombre and defuncion.responsable_dni:
            pdf.drawString(X_DATO, alto - 420, defuncion.responsable_nombre)
            pdf.drawString(395, alto - 662, defuncion.responsable_dni)
            pdf.drawString(340, alto - 677, defuncion.responsable_nombre)


    @staticmethod
    def generar_obito_pdf(obito, expediente, usuario):

        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'inline; filename="Obito-{obito.id}.pdf"'

        pdf = canvas.Canvas(response, pagesize=letter)
        pdf.setTitle(f"Obito-{obito.id}")
        ...
        # Etiquetas específicas de óbito
        ReportePacienteService._dibujarEtiquetasEstaticasObito(pdf, ancho, y)

        # Datos dinámicos específicos de óbito
        ReportePacienteService._dibujarDatosObito(
            pdf, obito, expediente, usuario, alto
        )

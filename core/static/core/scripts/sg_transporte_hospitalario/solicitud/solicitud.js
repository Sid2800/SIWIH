...
(function () {
	const tableNode = document.getElementById('tabla_solicitudes_th');
	if (!tableNode || !window.jQuery || !jQuery.fn.DataTable) {
		return;
	}

	const languageUrl = (document.currentScript && document.currentScript.dataset && document.currentScript.dataset.datatablesLanguageUrl) || '';
	const tabla = jQuery(tableNode).DataTable({
		order: [[1, 'desc']],
		language: {
			url: languageUrl || '/static/core/datatables/es-ES.json'
		}
	});

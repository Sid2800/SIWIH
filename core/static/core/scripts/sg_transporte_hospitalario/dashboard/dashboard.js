...
if (String(config.activeTab || '') !== 'resumen') {
return;
}

const tableNode = document.getElementById('th-resumen-table');
const periodoNode = document.getElementById('th-resumen-periodo');
const mesWrapNode = document.getElementById('th-resumen-mes-wrap');
const rangoWrapNode = document.getElementById('th-resumen-rango-wrap');
const modalSolicitud = document.getElementById('th-solicitud-modal');
const modalSolicitudContent = document.getElementById('th-solicitud-modal-content');
const modalSolicitudTitle = document.getElementById('th-solicitud-modal-title');
const modalSolicitudSubtitle = document.getElementById('th-solicitud-modal-subtitle');
const modalEjecucion = document.getElementById('th-ejecucion-modal');
const modalEjecucionContent = document.getElementById('th-ejecucion-modal-content');
const modalEjecucionTitle = document.getElementById('th-ejecucion-modal-title');
...
if (window.jQuery && jQuery.fn.DataTable && tableNode && resumenTieneFilasReales) {
	jQuery(tableNode).DataTable({
		order: [[1, 'desc']],
		language: {
			emptyTable: config.emptyResumenTable || 'No hay registros para los filtros seleccionados.'
		}
	});
}

// 2026-08-13: Resumen usa delegación de clics para no depender de onclick inline ni de namespaces globales.
if (tableNode) {
	tableNode.addEventListener('click', function (event) {
		const objetivo = event.target && event.target.closest ? event.target.closest('tbody tr') : null;
		if (!objetivo) {
			return;
		}
		...
		if (objetivo.dataset.solicitudId) {
			abrirModalSolicitud(objetivo);
			return;
		}
		if (objetivo.dataset.viajeId) {
			abrirModalViaje(objetivo);
		}
	});
}
...
async function requestGetJson(url) {
	...
	if (!response.ok) {
		throw new Error(data.error || data.message || data.detail || 'No se pudo cargar el detalle.');
	}
	return data;
}
...
function renderSolicitudDetalle(data) {
	const pacientes = Array.isArray(data.pacientes) ? data.pacientes : [];
	const personal = Array.isArray(data.personal) ? data.personal : [];
	const viajeAsociado = data.viaje_asociado || null;
	return [
		'<div class="th-solicitud-modal__hero">',
		'<div class="th-solicitud-modal__hero-main">',
		'<span class="th-solicitud-modal__eyebrow">Detalle histÃ³rico</span>',
		'<h5>Solicitud #' + escapeHtml(data.numero_solicitud || '-') + '</h5>',
		'<p>Consulta reutilizando el detalle del mÃ³dulo sin acciones de ediciÃ³n.</p>',
		'</div>',
		'</div>',
		...
		renderMetaItem('NÃºmero de solicitud', data.numero_solicitud || '-'),
		renderMetaItem('Fecha', data.fecha_solicitud || '-'),
		renderMetaItem('Estado', data.estado || '-'),
		renderMetaItem('Prioridad', data.prioridad || '-'),
		renderMetaItem('Punto de solicitud', data.punto_corto || data.punto_solicitud || '-', 'th-solicitud-modal__meta-item--wide'),
		renderMetaItem('Solicitante', data.solicitante && data.solicitante.nombre ? data.solicitante.nombre : '-'),
		'</div>',
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Viaje asociado</h5>',
		renderList(viajeAsociado ? [
			'<article class="th-solicitud-modal__item">',
			'<strong>Viaje #' + escapeHtml(viajeAsociado.numero_viaje || '-') + '</strong>',
			'<div>Estado: ' + escapeHtml(viajeAsociado.estado || '-') + '</div>',
			'</article>'
		] : [], 'Sin viaje asociado.'),
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Pacientes</h5>',
		renderList(pacientes.map(function (item) {
			return '<article class="th-solicitud-modal__item"><strong>' + escapeHtml(item.paciente || '-') + '</strong><div>' + escapeHtml(item.identidad || '-') + '</div><small>' + escapeHtml(item.sala || '-') + '</small></article>';
		}), 'Sin pacientes asociados.'),
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Personal solicitado</h5>',
		renderList(personal.map(function (item) {
			return '<article class="th-solicitud-modal__item"><strong>' + escapeHtml(item.nombre || '-') + '</strong><div>' + escapeHtml(item.cargo || '-') + '</div><small>' + escapeHtml(item.observacion || '-') + '</small></article>';
		}), 'Sin personal asociado.'),
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Motivo</h5>',
		renderList(data.motivo ? ['<div class="th-solicitud-modal__textblock">' + escapeHtml(data.motivo) + '</div>'] : [], 'Sin motivo registrado.'),
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>AnulaciÃ³n</h5>',
		renderList((data.motivo_anulacion || data.observacion_anulacion) ? [
			'<article class="th-solicitud-modal__item">',
			'<strong>' + escapeHtml(data.anulada_por || '-') + '</strong>',
			'<div>' + escapeHtml(data.anulada_en || '-') + '</div>',
			'<small>' + escapeHtml(data.motivo_anulacion || data.observacion_anulacion || '-') + '</small>',
			'</article>'
		] : [], 'Sin datos de anulaciÃ³n.'),
		'</section>',
		'</div>'
	].join('');
}

function renderViajeDetalle(row) {
	const detalle = parseDetalle(row);
	...
	return [
		'<div class="th-solicitud-modal__hero">',
		'<div class="th-solicitud-modal__hero-main">',
		'<span class="th-solicitud-modal__eyebrow">EjecuciÃ³n del viaje</span>',
		'<h5>Viaje #' + escapeHtml(row.dataset.viajeNumero || '-') + '</h5>',
		'<p>Vista completa de solo lectura con solicitudes, pacientes, personal y observaciones consolidadas.</p>',
		'</div>',
		'</div>',
		'<div class="th-solicitud-modal__grid">',
		'<section class="th-solicitud-modal__section">',
		'<h5>InformaciÃ³n general</h5>',
		'<div class="th-solicitud-modal__meta">',
		renderMetaItem('NÃºmero', infoGeneral.numero || row.dataset.viajeNumero || '-'),
		renderMetaItem('Estado', infoGeneral.estado_etiqueta || row.dataset.viajeEtapa || '-'),
		renderMetaItem('Tipo de viaje', infoGeneral.tipo_viaje || '-'),
		renderMetaItem('VehÃ­culo', infoGeneral.ambulancia || row.dataset.viajeVehiculo || '-'),
		renderMetaItem('Motorista', infoGeneral.motorista || row.dataset.viajeMotorista || '-'),
		'</div>',
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Salida</h5>',
		'<div class="th-solicitud-modal__meta">',
		renderMetaItem('Fecha/hora de salida', row.dataset.viajeFechaSalida || '-'),
		renderMetaItem('Kilometraje inicial', row.dataset.viajeKilometrajeSalida || '-'),
		renderMetaItem('Precio por litro', row.dataset.viajePrecioLitroSalida || '-'),
		renderMetaItem('Litros cargados', row.dataset.viajeLitrosCargadosSalida || '-'),
		renderMetaItem('Total combustible', row.dataset.viajeTotalCombustible || '-'),
		renderMetaItem('Observaciones', row.dataset.viajeObservacionesSalida || '-', 'th-solicitud-modal__meta-item--wide'),
		'</div>',
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Retorno</h5>',
		'<div class="th-solicitud-modal__meta">',
		renderMetaItem('Fecha/hora de retorno', row.dataset.viajeFechaRetorno || '-'),
		renderMetaItem('Kilometraje final', row.dataset.viajeKilometrajeRetorno || '-'),
		renderMetaItem('Observaciones', row.dataset.viajeObservacionesRetorno || '-', 'th-solicitud-modal__meta-item--wide'),
		'</div>',
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Solicitudes consolidadas</h5>',
		renderList(solicitudes.map(function (item) {
			const motivosTexto = Array.isArray(item.motivos) && item.motivos.length ? item.motivos.join(' Â· ') : 'Sin motivo';
			return '<article class="th-solicitud-modal__item"><strong>Solicitud #' + escapeHtml(item.numero || '-') + '</strong><div>Punto: ' + escapeHtml(item.punto || '-') + '</div><div>Prioridad: ' + escapeHtml(item.prioridad || '-') + '</div><small>' + escapeHtml(motivosTexto) + '</small></article>';
		}), 'No hay solicitudes consolidadas en esta ejecuciÃ³n.'),
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Pacientes</h5>',
		renderList(pacientes.map(function (item) {
		...
		renderList(motivos.map(function (item) {
			const motivosTexto = Array.isArray(item.items) && item.items.length ? item.items.join(' Â· ') : 'Sin motivo';
			return '<article class="th-solicitud-modal__item"><strong>Solicitud #' + escapeHtml(item.numero || '-') + '</strong><div>' + escapeHtml(motivosTexto) + '</div></article>';
		}), 'No hay motivos consolidados.'),
		'</section>',
		'<section class="th-solicitud-modal__section">',
		'<h5>Observaciones</h5>',
		renderList(observaciones.map(function (item) {
...
function abrirModalSolicitud(row) {
	if (!modalSolicitud || !modalSolicitudContent) return;
	const id = row ? String(row.dataset.solicitudId || '') : '';
	if (!id) return;
	modalSolicitudContent.innerHTML = '<div class="th-solicitud-modal__loading"><i class="bi bi-hourglass-split"></i> Cargando detalle...</div>';
	modalSolicitud.classList.add('is-open');
	modalSolicitud.setAttribute('aria-hidden', 'false');
	document.body.classList.add('th-modal-open');
	if (modalSolicitudTitle) modalSolicitudTitle.textContent = 'Detalle de solicitud';
	if (modalSolicitudSubtitle) modalSolicitudSubtitle.textContent = 'Consulta histÃ³rica en modo solo lectura';
	void requestGetJson(config.urlDetalleSolicitud + '?id=' + encodeURIComponent(id)).then(function (respuesta) {
		modalSolicitudContent.innerHTML = renderSolicitudDetalle(respuesta.data || {});
	}).catch(function (error) {
		modalSolicitudContent.innerHTML = '<div class="th-solicitud-modal__empty">' + escapeHtml(error.message || 'No se pudo cargar el detalle de la solicitud.') + '</div>';
	});
}

function cerrarModalSolicitud() {
	if (!modalSolicitud || !modalSolicitudContent) return;
	modalSolicitud.classList.remove('is-open');
	modalSolicitud.setAttribute('aria-hidden', 'true');
	modalSolicitudContent.innerHTML = '<div class="th-solicitud-modal__empty">Seleccione una solicitud para ver su detalle.</div>';
	document.body.classList.remove('th-modal-open');
}

function abrirModalViaje(row) {
	if (!modalEjecucion || !modalEjecucionContent) return;
	modalEjecucionContent.innerHTML = renderViajeDetalle(row);
	modalEjecucion.classList.add('is-open');
	modalEjecucion.setAttribute('aria-hidden', 'false');
	document.body.classList.add('th-modal-open');
	if (modalEjecucionTitle) modalEjecucionTitle.textContent = 'EjecuciÃ³n del viaje';
	if (modalEjecucionSubtitle) modalEjecucionSubtitle.textContent = 'Detalle histÃ³rico en modo solo lectura';
	if (modalEjecucionDecision) modalEjecucionDecision.textContent = 'Consulta histÃ³rica en modo solo lectura.';
}

function cerrarModalViaje() {
	if (!modalEjecucion || !modalEjecucionContent) return;
	modalEjecucion.classList.remove('is-open');
	modalEjecucion.setAttribute('aria-hidden', 'true');
	modalEjecucionContent.innerHTML = '<div class="th-solicitud-modal__empty">Seleccione un viaje para ver su detalle.</div>';
	document.body.classList.remove('th-modal-open');
}

if (modalSolicitud) {
	modalSolicitud.addEventListener('click', function (event) {
		const cierre = event.target && event.target.closest ? event.target.closest('[data-modal-close]') : null;
		if (cierre) {
...
if (modalEjecucion) {
	modalEjecucion.addEventListener('click', function (event) {
		const cierre = event.target && event.target.closest ? event.target.closest('[data-modal-close]') : null;
		if (cierre) {
			cerrarModalViaje();
		}
...
function actualizarPeriodoUI() {
	const esMes = !periodoNode || periodoNode.value !== 'rango';
	if (mesWrapNode) {
		mesWrapNode.style.display = esMes ? 'grid' : 'none';
	}
	if (rangoWrapNode) {
		rangoWrapNode.style.display = esMes ? 'none' : 'grid';
	}
}

if (periodoNode) {
	periodoNode.addEventListener('change', actualizarPeriodoUI);
	actualizarPeriodoUI();
}

const resumenForm = document.querySelector('.th-resumen__filters');
const resumenMesInicio = document.getElementById('th-resumen-mes-inicio');
const resumenMesFin = document.getElementById('th-resumen-mes-fin');
...
function validarRangoResumen() {
	if (!resumenMesInicio || !resumenMesFin) {
		return true;
	}
	resumenMesFin.setCustomValidity('');
	const inicio = parseMonthValue(resumenMesInicio.value);
	const fin = parseMonthValue(resumenMesFin.value);
	if (inicio && fin && fin < inicio) {
		resumenMesFin.setCustomValidity('El mes final no puede ser anterior al mes inicio.');
		return false;
	}
	return true;
}
...
if (resumenMesInicio) {
	resumenMesInicio.addEventListener('change', validarRangoResumen);
	resumenMesInicio.addEventListener('input', validarRangoResumen);
}
if (resumenMesFin) {
	resumenMesFin.addEventListener('change', validarRangoResumen);
	resumenMesFin.addEventListener('input', validarRangoResumen);
}
if (resumenForm) {
	resumenForm.addEventListener('submit', function (event) {
		if (!validarRangoResumen()) {
			event.preventDefault();
			if (resumenMesFin && typeof resumenMesFin.reportValidity === 'function') {
				resumenMesFin.reportValidity();
			}
		}
	});
}
...
'<div class="th-solicitud-modal__grid">',
'<section class="th-solicitud-modal__section">',
'<h5>InformaciÃ³n del viaje</h5>',
'<div class="th-solicitud-modal__meta">',
'<div class="th-solicitud-modal__meta-item"><span>NÃºmero</span><strong>' + escapeHtml(data.numero || '-') + '</strong></div>',
'<div class="th-solicitud-modal__meta-item"><span>Fecha</span><strong>' + escapeHtml(data.fecha || '-') + '</strong></div>',
'<div class="th-solicitud-modal__meta-item"><span>VehÃ­culo</span><strong>' + escapeHtml(data.vehiculo || '-') + '</strong></div>',
...
'<h5>InformaciÃ³n general</h5>',
'<div class="th-solicitud-modal__meta">',
renderLineaMeta('NÃºmero', infoGeneral.numero || data.numero || '-'),
...
renderLineaMeta('VehÃ­culo', infoGeneral.ambulancia || data.vehiculo || '-'),
...
'<section class="th-solicitud-modal__section">',
'<h5>Solicitudes consolidadas</h5>',
renderListaHtml(solicitudes, 'No hay solicitudes consolidadas en esta ejecuciÃ³n.', function (item) {
return [
'<article class="th-solicitud-modal__item">',
...
'<div class="th-solicitud-modal__focus">' + escapeHtml(Array.isArray(item.motivos) && item.motivos.length ? item.motivos.join(' Â· ') : 'Sin motivo') + '</div>',
'</article>'
].join('');
}),
'</section>',
...
const motivoTexto = Array.isArray(item.items) ? item.items.join(' Â· ') : '';
...
document.body.classList.remove('th-modal-open');
modalContent.innerHTML = '<div class="th-solicitud-modal__empty">Seleccione un viaje para ver o registrar su ejecuciÃ³n.</div>';
...
if (!response.ok) {
throw new Error(data.error || data.message || data.detail || 'No se pudo guardar la ejecuciÃ³n.');
}
return data;
}
...
if (window.Swal) {
await Swal.fire({ icon: 'error', title: 'No fue posible guardar la ejecuciÃ³n', text: error.message || 'No se pudo guardar.' });
} else {
